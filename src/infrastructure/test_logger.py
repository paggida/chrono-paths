...
class TestLogger:
    ...
    def test_returns_a_formatted_message(self, logger):
        message = fake.text()
        formatted_message = logger._get_log_message(message)

        assert formatted_message == message

    def test_returns_a_formatted_message_with_full_message(self, logger):
        message = fake.text()
    ...
    def test_returns_a_formatted_message_with_operation(self, logger):
        message = fake.text()
    ...
    def test_returns_a_full_formatted_message(self, logger):
        message = fake.text()
        full_message = fake.text()
        operation = fake.pystr()
